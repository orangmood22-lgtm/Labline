...
caller: executor
platform: both
status: needs-safety-confirmation
consumes:
  - project.yaml
examples:
  - "/sync push"
...
- `git push`、首次 `git push -u`、`rsync --delete`、修改 remote URL、写入 token/API 配置都必须先展示计划并获得用户明确确认。
- `status`、本地 `git add/commit`、无 `--delete` 的 dry-run rsync 可以自动执行。
- 如果当前运行环境无法确认用户授权，停止在计划阶段，不执行远端写操作。
