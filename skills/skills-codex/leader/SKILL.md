...
- 所有派生 agent 必须遵循 `skills/shared-references/runtime-task-protocol.md`；Leader 派替代任务或接受替代产物后，必须为旧 task 写 `task.superseded` / `task.resolved_by`，或写带终态 status 的 `leader.decision`。
- Leader 派发前先登记或更新 Runtime Task，写入 role、`next_expected_update`、required artifact，Reviewer 还必须声明 verdict artifact；每个派生 prompt 必须显式注入 Runtime Task Contract。
- 如果同一 gate 的 agent 已出现 `observability_failure=true`、`boot_no_progress`、`handle returned not_found`、或 `starting` 超过 `next_expected_update` 且无 `job_handles`，Leader 不得继续用同一条裸 `spawn_agent` / `local_agent` 通道重试。下一次必须改变可观测 transport：优先前台独立 review transport（例如 `cli_session` / Codex exec / MCP reviewer），或只有在派发后 120 秒内拿到可持久化 JSON `job_handles` 且看到 agent status start/update 时，才允许继续 delegated agent。
...
当 Leader 运行在飞书 / Lark bridge 里时，用户看到的是一张当前 turn 的运行卡片。Leader 必须避免把这张卡片长时间保持在“正在输出”。

- 预计超过 3 分钟的任务都按长任务处理：环境安装、编译、下载、训练、部署、批量评估、长时间 `wait_agent`。
...
- 只有真正的短 gate（预计 120 秒内完成的代码审查、小测试、只读状态检查）才允许前台 `wait_agent` 等到最终结果。
- 正常运行中不连续推送刷屏；只有 completed、failed、cancelled、blocked、need_decision、anomaly 或 heartbeat escalation 这类需要用户处理的状态才通过 heartbeat/monitor/最终回复推送。
...
5. 确认 Reviewer 可用：默认用独立 reviewer agent；如项目配置了 MCP reviewer bridge，可额外做 bridge health check。Reviewer 不可用时警告降级，但不要让 Executor 自审通过。
...
**送 Reviewer 审核计划：** 详见 `.agents/skills/shared-references/reviewer-routing.md` 和 `.agents/skills/shared-references/reviewer-independence.md`。
...
Agent:
  ...
  prompt: |
    ...
    Read .agents/skills/coder/SKILL.md 了解 Coder 职责边界。
    Read .agents/skills/shared-references/agent-status-stream.md 了解状态汇报协议。
    Read .agents/skills/shared-references/runtime-task-protocol.md 了解终态和替代任务协议。
    ...
    参考：.agents/skills/shared-references/executor-skill-routing.md
...
Coder 完成后，**送 Reviewer 审查代码：** 详见 `.agents/skills/shared-references/reviewer-routing.md` 和 `.agents/skills/shared-references/reviewer-independence.md`。
...
Agent:
  ...
  prompt: |
    ...
    ## 首先
    Read .agents/skills/shared-references/agent-guide.md 了解可用 skills 和约束。
    Read .agents/skills/deployer/SKILL.md 了解 Deployer 职责边界。
    Read .agents/skills/shared-references/agent-status-stream.md 了解状态汇报协议。
    Read .agents/skills/shared-references/runtime-task-protocol.md 了解 job handle、终态和替代任务协议。
    ...
    - 遵循飞书远控长任务规则：预计超过 3 分钟时写 runtime/agent status + job handle，Leader 最多短等 120 秒后收口，不长时间占住卡片
...
Agent:
  ...
  prompt: |
    ...
    - 每个长运行任务必须写 job handle 和 next_expected_update
    - 禁止 tail 轮询
```
...
**送 Reviewer 独立审计：** 详见 `.agents/skills/shared-references/reviewer-routing.md` 和 `.agents/skills/shared-references/reviewer-independence.md`。
...
**送 Reviewer 判定 claim：** 详见 `.agents/skills/shared-references/reviewer-routing.md` 和 `.agents/skills/shared-references/reviewer-independence.md`。
...
Agent:
  ...
  prompt: |
    ...
    ##首先
    Read .agents/skills/shared-references/agent-guide.md 了解可用 skills 和约束。
    Read .agents/skills/writer/SKILL.md 了解 Writer 职责边界。
    Read .agents/skills/shared-references/agent-status-stream.md 了解状态汇报协议。
    Read .agents/skills/shared-references/runtime-task-protocol.md 了解终态和替代任务协议。
