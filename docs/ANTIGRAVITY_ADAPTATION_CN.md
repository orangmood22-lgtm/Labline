# Antigravity 适配指南（Labline 工作流）

> 在 **Google Antigravity** 中使用 Labline 研究工作流 —— Google DeepMind 出品的 Agent-First AI IDE。

Antigravity 原生支持与 Labline 相同的 `SKILL.md` 格式（YAML frontmatter + Markdown body），是 Labline 工作流最自然的宿主之一。
...
Antigravity 支持多种模型作为**执行器**（运行 Labline 工作流的模型）：
...
Labline 使用外部 LLM（GPT-5.4 via Codex）作为审阅者。在 Antigravity 中启用：
...
Antigravity 通过 `SKILL.md` 中的 YAML `description` 字段自动发现 Labline 技能。三种方式：
...
如果 Labline 技能已安装（§3.1），Antigravity 会自动发现并激活 `auto-review-loop` 技能。
...
### 多 Agent 编排
使用 Antigravity 的 **Manager View** 同时运行多个 Labline 阶段：
...
### Artifact 系统
Labline 产出自然映射到 Antigravity 的 artifact 系统：
