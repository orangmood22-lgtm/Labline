# Labline Trae 适配指南（Workflow Runbook）

在 Trae 中使用 Labline 研究工作流，不依赖 Claude Code 的 `/skill-name` 斜杠命令。
...
| Skill 存放 | `~/.claude/skills/...` | 全局 `~/.trae/skills/`（跨项目可用）或项目 `<project>/.trae/skills/`（仅当前项目），或直接引用 Labline 仓库 `skills/` |
...
## 2. Setup
最好在trae中创建一个单独的智能体负责运行Labline工作流，避免与其他智能体冲突，并能给Labline工作流提供扮演角色的必要信息。
### 2.1 克隆仓库并配置 Skills
...
1. 进入 `设置 → 规则和技能`
2. 选择「全局」或「项目」安装范围
3. 点击「导入文件」，选择 Labline 仓库中的 `skills/` 目录下的 SKILL.md 文件
4. 安装后即可通过自然语言描述触发技能
...
Copy-Item -Path ".\Auto-research-in-sleep\skills\*" -Destination $globalSkillsDir -Recurse -Force

# 项目级安装（仅当前项目可用）
$projectSkillsDir = ".\.trae\skills"
New-Item -ItemType Directory -Path $projectSkillsDir -Force | Out-Null
Copy-Item -Path ".\Auto-research-in-sleep\skills\*" -Destination $projectSkillsDir -Recurse -Force
```
...
Labline 的关键机制是"执行模型 + 外部审阅模型"。先配好审阅 MCP，再跑流程。
...
```powershell
cd D:\path\to\Auto-research-in-sleep
python -m venv .venv
...
```json
{
  "mcpServers": {
    "llm-chat": {
      "command": "/path/to/Auto-research-in-sleep/.venv/Scripts/python.exe",
...
注意：使用 `@skills/.../SKILL.md` 时，对应的 `skills/` 目录必须在当前 Trae workspace 中可见（例如把 Labline 仓库或其中的 `skills/` 目录加入当前工作区），否则文件引用会失败。
### D. 高频技能固化为本地规则
...
Trae 通过 `SKILL.md` 中的 YAML `description` 字段自动发现 Labline 技能。以下是各工作流的调用方式：
...
| Labline MCP 工具 | 作用 | 需要的 MCP Server |
|---|---|---|
...
和 Labline 原流程一致，在项目说明里提供服务器信息，然后调用：
...
- [ ] 导入 Labline skills 的 SKILL.md 文件
