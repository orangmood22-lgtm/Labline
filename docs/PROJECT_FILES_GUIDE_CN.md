...
> Labline 科研工作流中的项目级状态文件规范 — 每个文件的定位、写入时机、以及相互关系。

## 问题

Labline 工作流会在多个阶段产出大量信息：idea、实验计划、结果、审稿反馈、决策。如果没有清晰的文件规范，这些信息会散落在聊天记录中，在上下文压缩或新会话时丢失。
...
Labline 技能使用带时间戳的文件名保留历史。每次产出写入两份：
