...
- （可选）Claude Code + Anthropic API Key；Labline 保持兼容，但不是默认入口
...
```bash
git clone https://github.com/orangmood22-lgtm/Labline.git [你的framework位置]
```

如果想把 Labline 当普通 CLI 用，建议把命令链接到 `PATH`：

```bash
mkdir -p ~/.local/bin
ln -sf [你的framework位置]/tools/lane ~/.local/bin/lane
export PATH="$HOME/.local/bin:$PATH"
```

之后文档里的 `lane ...` 等价于 `[你的framework位置]/tools/lane ...`。

### 2. 创建科研项目

```bash
mkdir -p ~/projects/my-research && cd ~/projects/my-research
lane project init . --direction "你的研究方向"
```
...
- `.labline/manifest.json` — CLI 初始化记录
...
```bash
cd ~/projects
lane project init ./my-research --direction "你的研究方向"
```

已有项目也用同一个命令半路接入 Labline；现有代码、数据、论文、git 历史会保留：
...
进入后用 `$skill-name` 调用 Labline skills。Claude Code 用 `/skill-name`。
...
- 框架 `lane framework update` → 更新 framework 并默认同步已登记项目
- 新增 skill → 项目内运行 `lane project update`
- 移除 Labline 接入但保留项目文件 → `lane project detach`
...
| 命令 | 作用 |
|------|------|
| `lane project init . --direction "方向"` | 当前目录初始化项目 |
| `lane project init PATH --direction "方向"` | 创建/接入指定目录 |
| `lane project doctor` | 检查项目初始化状态 |
| `lane project update` | 重建项目 Labline 接入 |
| `lane project detach` | 移除 Labline 接入，保留项目文件 |
| `lane framework --version` | 查看 framework 版本 |
| `lane framework check-update` | 检查 framework 是否有更新，不改变本地版本 |
| `lane framework update` | 更新 framework 并同步登记项目 |
| `lane framework rollback` | 回退上一次 framework 更新 |
