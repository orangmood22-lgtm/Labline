# Labline CLI 与部署运行时机制说明

> 创建时间: 2026-06-16
> 适用对象: Labline framework 开发者、部署维护者、release reviewer
> 相关模块: `tools`, `deploy`, `templates`, `docs`, `tests`

本文档说明当前 Labline CLI 与容器部署运行时的内部机制。用户侧命令和操作步骤以 `QUICK_START.md`、`docs/OPERATIONS_GUIDE.md`、`deploy/DEPLOY_GUIDE.md` 为准；本文只解释实现边界、状态文件、调用链和维护约束。

## 设计目标

Labline 当前 CLI/部署层解决四个问题：
...
- `lane project ...` 管项目。
- `lane framework ...` 管用户自己的 framework copy。
...
托管部署中，管理员指定一个 `LABLINE_ROOT`，每个用户一个 workspace：

```text
[LABLINE_ROOT]/
├── users/<user>/
│   ├── framework/   # 用户自己的 Labline checkout
│   ├── projects/    # 用户项目工作区
│   └── .labline/       # 用户级 Labline 非版本化状态
└── shared/
    ├── datasets/
    ├── pretrained/
    └── downloads/
...
开发者不要把 `/lane/...` 误认为宿主机路径；它是容器内 ABI。宿主机路径由 `.env` 中的 `USERn_FRAMEWORK_PATH`、`USERn_PROJECTS_PATH`、`USERn_STATE_PATH`、`DATASETS_PATH` 等决定。
...
lane framework check-update [--if-stale DURATION] [--notify] [--no-fetch]
lane framework update [--no-project-sync]
lane framework rollback [--no-project-sync]
```

旧短命令 `lane init`、`lane update`、`lane doctor`、`lane detach` 不支持。当前没有老用户，故没有兼容层；测试会锁住这一点。

## 路径发现机制

`tools/lane` 的 framework 解析顺序：

1. 显式参数 `--lane-repo`。
2. `Labline_WORKSPACE/framework`，如果 `Labline_WORKSPACE` 存在且该目录存在。
3. 当前 CLI 所在 repo，即 `tools/lane` 的上级目录。

workspace 解析顺序：

1. 环境变量 `Labline_WORKSPACE`。
2. 如果 CLI 位于 `.../framework/tools/lane`，则 workspace 是 `.../framework` 的父目录。
3. 如果 `/lane/framework` 或 `/lane/projects` 存在，则 workspace 是 `/lane`。
...
1. 创建或选择 `PATH`。
2. 创建标准项目目录。
3. 写 `.gitignore`，避免 outputs、模型权重、`.labline/`、`.agents/`、`.claude/` 入库。
4. 如果没有 `.git/`，执行 `git init`。
5. 调用底层安装器：
   - `tools/install_labline.sh` 写 Claude Code 兼容接入。
   - `tools/install_labline_codex.sh` 写 Codex 接入。
6. 从 templates 写入：
   - `project.yaml`
   - `AGENTS.md`
   - `CLAUDE.md`
7. 写项目级 manifest：
   - `<project>/.labline/manifest.json`
...
```text
<workspace>/.labline/project-registry.json
```
...
`lane project detach [PATH]` 语义是“移除 Labline 接入但保留项目内容”。

当前 detach 会：

- 调用安装器的 `--uninstall` 删除已记录的 skill links。
- 删除 `.labline/manifest.json`、安装 manifest。
...
```text
<workspace>/.labline/framework-history/previous
```
...
1. 读取 `<workspace>/.labline/framework-history/previous`。
...
```text
<workspace>/.labline/framework-update-status.json
```
...
- 当前提醒文本只告诉用户运行 `lane framework update` 和 rollback 命令，不自动执行。
...
2. 写 proxy 配置。
3. 拷贝 SSH key。
4. 建立 `~/.local/bin/lane -> /lane/framework/tools/lane`。
5. 写 `~/.labline/lane-shell-hook.sh`。
...
```text
USERn_FRAMEWORK_PATH -> /lane/framework
USERn_PROJECTS_PATH  -> /lane/projects
USERn_STATE_PATH     -> /lane/.labline
DATASETS_PATH        -> /lane/shared/datasets:ro
PRETRAINED_PATH      -> /lane/shared/pretrained
DOWNLOADS_PATH       -> /lane/shared/downloads
Labline_WORKSPACE       -> /lane
```

不要恢复共享 framework named volume，例如：

```text
lane-framework:/lane/framework
```

不要恢复共享项目 volume，例如：

```text
user1-projects:/lane/projects
```

每个用户的 framework copy 是版本隔离边界；每个用户的 `.labline` 是 registry、rollback history、update status 的状态边界。
...
python -m py_compile tools/lane
```
