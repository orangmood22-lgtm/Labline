...
rm -rf /path/to/labline-dev/skills/<new-skill>
```
...
| 修改 install_labline.sh | 核心工具，谨慎修改，必须经过完整测试 |
...
```bash
# 查看所有 commit
git -C /path/to/labline-stable log --oneline

# 查看某文件历史
git -C /path/to/labline-stable log --oneline -- tools/install_labline.sh

# 生成 changelog 草稿（按 tag 分组）
git -C /path/to/labline-stable log --pretty=format:"%s" $(git describe --tags --abbrev=0)..HEAD
```
