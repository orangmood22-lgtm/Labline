...
- 默认 smoke 只验证 ARIS CLI 基础链路：`framework --version`、`project init`、`project doctor`、`project update`。
...
```bash
docker compose --env-file to-developer/realtest/.env.test -f to-developer/realtest/docker-compose.test.yaml up --build aris-realtest
```

运行自定义命令：

```bash
ARIS_REALTEST_COMMAND='python -m pytest tests/test_feishu_transport_docs.py -q' \
docker compose --env-file to-developer/realtest/.env.test -f to-developer/realtest/docker-compose.test.yaml up --build aris-realtest
```
...
- 默认关闭自动更新检查：`ARIS_AUTO_CHECK_UPDATE=0`，避免测试运行隐式修改 framework checkout。
