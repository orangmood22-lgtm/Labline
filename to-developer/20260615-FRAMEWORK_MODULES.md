...
| `tools` | `tools/` | 本地 CLI/脚本工具，包括文档 DAG、release、状态流、飞书 session runner、安装/同步辅助。 | 对应测试、README/工具索引、开发日志；涉及 Codex session/thread/CLI 语义时先核对 Codex CLI/app-server 原生接口，再决定是否补 Labline wrapper。 |
...
| 为 Codex session/thread/fork/resume/name 等能力补入口 | 先查 Codex CLI/app-server；若只做 Labline 命令、审计或 bridge 绑定，归入 `tools`，必要时同步 `docs`/`to-developer` |
