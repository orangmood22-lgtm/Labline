...
| P2 | Workflow Runtime Stategraph | `/follow`、heartbeat、escalation 和 lease 已具备前置条件，当前应先冻结 auto-wakeup 状态图和 adapter 边界 |
| P3 | LangGraph Workflow Engine | 长期重要；当前进入可选 backend spike，但不能替代 Labline runtime truth |
| P3 | OpenClaw / Cline SDK / OpenHands SDK | 长期 orchestration/runtime 候选；当前只按 Labline 状态图做 adapter 评估 |
...
开发新功能时，优先查被集成系统已经暴露的稳定接口，再决定是否在 Labline 侧补 adapter。不要先从外层 bridge、shim 或自研状态文件反推能力。

默认顺序:

1. **Codex CLI / Codex app-server**: 涉及 session、thread、fork、resume、archive、rename、turn、model、approval、sandbox、tool stream 等 Codex 原生语义时，先查 `codex --help`、相关子命令 help、`codex app-server generate-ts/json-schema` 和官方 Codex manual。若已有 `codex fork`、`thread/fork`、`thread/name/set` 这类接口，Labline 应优先包一层可审计入口，而不是重造 transcript/context 复制逻辑。
2. **上游 transport / bridge**: 再查 `lark-channel-bridge`、MCP server、SDK 或外部服务是否已支持该能力。这里判断的是消息传输、卡片、回调、权限、OAuth、日志和进程管理，不改变 Codex 原生语义。
3. **Labline adapter / wrapper**: 若上游有能力但缺少 Labline 入口，补 `lane ...`、bridge patch、Remote Observation routing、runtime event 或文档约束；adapter 只做绑定、审计、投影、权限和用户体验。
4. **Labline 自研协议**: 只有当 Codex/上游没有合适接口，或 Labline 需要跨 transport 的稳定项目真相时，才新增 `.labline/runtime/` 协议、control intent、lease、heartbeat 或 workflow state。
5. **后置 workflow engine**: LangGraph、OpenClaw、Cline SDK、OpenHands SDK 等只应消费已稳定的 Labline runtime/adapter 契约；不能为了接入 runtime 候选而改写核心语义。

判断标准:

- 能用 Codex 原生 thread/session API 表达的，不用聊天记录摘要伪造。
- 能用上游 bridge 的卡片/回调/会话能力表达的，不在 Labline 里复制传输层。
- Labline 只在需要跨端审计、权限收敛、项目 runtime 真相或用户侧命令稳定性时加一层。
- 如果文档和本机可调用能力冲突，先记录版本、命令输出和 schema，再决定是否做兼容分支。
...
- 先冻结 `to-developer/plans/20260704-WORKFLOW_RUNTIME_STATEGRAPH_SPIKE.md` 中的 auto-wakeup 状态图。
- `heartbeat/escalation/follow` 只产生状态和推送，不直接启动 Leader。
- 后续先做 native `wakeup-plan --dry-run`，再决定是否接 LangGraph backend。
- OpenClaw 只作为 agent runtime / coding orchestration adapter 候选，不能拥有 `.labline/runtime/` 真相。
...
- 是否已有 Codex CLI / Codex app-server 原生接口可用?
- 是否已有上游 bridge / SDK / MCP 接口可用，Labline 只需要 adapter?
- 运行状态写在哪里?
- 是否需要更新 `CONTEXT.md`?
